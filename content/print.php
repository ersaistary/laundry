...
<body>
  ...
  <?php foreach($rowSelectService as $item): ?>
    <div class="item-block">
      ...
      <div class="row-inline">
        <span><?= $item['qty'] / 1000 ?> Kg</span>
        <span style="float: right;">Rp <?= number_format($item['subtotal']/1000, 0, ',', '.') ?></span>
      </div>
    </div>
  <?php endforeach; ?>
  ...
  <hr>
  <div class="summary">
    <p style="text-align:right;">Total: Rp <?= number_format($rowDetail['total']/1000, 0, ',', '.') ?></p>
    <p style="text-align:right;">Dibayar: Rp <?= number_format($rowDetail['order_pay']/1000, 0, ',', '.') ?></p>
    <p style="text-align:right;">Kembalian: Rp <?= number_format($rowDetail['order_change']/1000, 0, ',', '.') ?></p>
  </div>
  <hr>
