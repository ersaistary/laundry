<?php
  // … kode sebelumnya (session, koneksi, helper, dsb) …

  // 0) Baca periode dari GET, default bulan ini
  $periode = isset($_GET['periode']) 
           ? $_GET['periode']       // format “YYYY-MM”
           : date('Y-m');
  ...
  <table class="table table-striped">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="text-right"><?= rupiah($row['total']/1000) ?></td>
        <td class="text-right"><?= rupiah($row['order_pay']/1000) ?></td>
    ...
    <tfoot>
      <tr>
        ...
        <th class="text-right"><?= rupiah($sumTotal/1000) ?></th>
        <th class="text-right"><?= rupiah($sumPay/1000) ?></th>
        <th class="text-right"><?= rupiah($sumChange/1000) ?></th>
        <th></th>
      </tr>
    </tfoot>
  </table>

  <p style="margin-top:2rem">Dicetak pada: <?= date('d/m/Y H:i') ?></p>
